    ...
    public boolean func_77644_a(ItemStack stack, EntityLivingBase target, EntityLivingBase attacker) {
        if (target instanceof EntityWitherTurretGround) {
            if (((EntityWitherTurretGround)target).isPlayerCreated()) {
                ...
                if (attacker != null && itemstack != null && itemstack.func_77973_b() == TitanItems.goodTurret2 && !((EntityWitherTurretGround)target).field_70170_p.field_72995_K) {
                    ((EntityWitherTurretGround)target).func_70106_y();
                    ItemStack goodTurret = new ItemStack(TitanItems.goodTurret2);
                    if (((EntityWitherTurretGround)target).durabilityLevel > 0) {
                        goodTurret.func_77966_a(TheTitans.turretEnchant1, ((EntityWitherTurretGround)target).durabilityLevel);
                    }

                    if (((EntityWitherTurretGround)target).ferocityLevel > 0) {
                        goodTurret.func_77966_a(TheTitans.turretEnchant2, ((EntityWitherTurretGround)target).ferocityLevel);
                    }

                    if (((EntityWitherTurretGround)target).maniacLevel > 0) {
                        goodTurret.func_77966_a(TheTitans.turretEnchant3, ((EntityWitherTurretGround)target).maniacLevel);
                    }

                    if (((EntityWitherTurretGround)target).unstabilityLevel > 0) {
                        goodTurret.func_77966_a(TheTitans.turretEnchant4, ((EntityWitherTurretGround)target).unstabilityLevel);
                    }

                    if (((EntityWitherTurretGround)target).shurakinLevel > 0) {
                        goodTurret.func_77966_a(TheTitans.turretEnchant5, ((EntityWitherTurretGround)target).shurakinLevel);
                    }

                    if (((EntityWitherTurretGround)target).unbreakingLevel > 0) {
                        goodTurret.func_77966_a(Enchantment.field_77347_r, ((EntityWitherTurretGround)target).unbreakingLevel);
                    }

                    if (!((EntityPlayer)attacker).field_71075_bZ.field_75098_d) {
                        ((EntityWitherTurretGround)target).func_70099_a(goodTurret, 1.0F);
                    }
                }
            }
